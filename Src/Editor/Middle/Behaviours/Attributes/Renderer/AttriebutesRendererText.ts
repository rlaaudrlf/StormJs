...
@DefineAttributesRenderer(RendererText)
export class AttriebutesRendererText extends AttributesBase {
	namearea;
	render(transform: Transform, render: RendererText) {
		...
		let binder = stack.addBehaviour(Binder);
		...
		insobj.transfrom.Parent = stack.transfrom;
		insobj.transfrom.Width = 60;
		...
		this.namearea.setCompData(render.text);
		binder.pathcer.Add("text", this.namearea);
        binder.setCompData(render);
	}
}
